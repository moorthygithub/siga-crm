...
const PageControl = () => {
    ...
    const { data: routeControl, isLoading, error } = useQuery({
      ...
      queryFn: async () => {
        ...
        const response = await axios.get(
          'https://southindiagarmentsassociation.com/public/api/panel-fetch-usercontrol-new',
          {
            headers: {
    ...
    const updateMutation = useMutation({
      mutationFn: async ({ id, updatedData }) => {
        ...
        const response = await axios.put(
          `https://southindiagarmentsassociation.com/public/api/panel-update-usercontrol-new/${id}`,
          updatedData,
          {
            headers: {
              'Authorization': `Bearer ${token}`,
                     ...
                     {userTypes.map((user) => (
                       <td key={user.id} className="px-4 py-3 text-sm">
                         <Checkbox
                           ...
                           onCheckedChange={() => handlePermissionChange({ url: `/${route.url}` }, user.id)}
                           disabled={columnSelections[`/${route.url}`]?.[user.id] || updateMutation.isLoading}
                           className={columnSelections[`/${route.url}`]?.[user.id] ? "opacity-50" : ""}
                         />
                       </td>
                     ))}
                   </tr>
