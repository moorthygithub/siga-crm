...
const BusinessOppList = () => {
  ...
  const navigate = useNavigate()
    ...
    useEffect(() => {
      const lastEditedId = localStorage.getItem("lastEditedBusinessId");
  
      if (lastEditedId) {
        setHighlightedRowId(parseInt(lastEditedId));
  
        localStorage.removeItem("lastEditedBusinessId");
  
        setTimeout(() => {
          const element = document.getElementById(`business-row-${lastEditedId}`);
          if (element) {
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        return (
          <div className='w-36 break-words'>{row.getValue("full_name")}</div>
        )
      },
    },
    {
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return (
          ...
          onClick={()=>
          {
            localStorage.setItem("lastEditedBusinessId", registration);
            navigate(`/business-opp-edit/${registration}`)}
          }
          ...
          <BuisnessView
           ...
           {
            localStorage.setItem("lastEditedBusinessId", registration);
            navigate(`/business-opp-view/${registration}`)}
           }
                      ...
                      {table.getRowModel().rows?.length ? (
                        table.getRowModel().rows.map((row) => (
                          <TableRow
                            ...
                            id={`business-row-${row.original.id}`}
                            className={
        ...
        {/* row slection and pagintaion button  */}
         <div className="flex items-center justify-end space-x-2 py-4">
                <div className="flex-1 text-sm text-muted-foreground">
                  Total {table.getFilteredRowModel().rows.length} Business(s).
                </div>
              </div>
      </div>
   </Page>
